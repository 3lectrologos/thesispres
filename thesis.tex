...
F(S) &= \sum_{i \in S} u_i + \sum_{j=1}^{L} \left(\max_{i \in S} w_{ij} - \sum_{i \in S} w_{ij}\right),\ \ \ u_i \in \mathbb{R}, w_{ij} \in \mathbb{R}_{\geq 0}
\end{align*}
\end{frame}
...
\begin{itemize}
  \item Learn cancer type ``factorization"
  \vspace{2em}
  \item Learn models that are guaranteed to mix fast
\end{itemize}
\end{frame}
