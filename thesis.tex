...
\begin{frame}{Solution II: Trivial imputation}
  ...
  \vspace{1em}
  \uncover<2->{%
  May be able to filter it out (more on this later), but\ldots
  \begin{itemize}
	\vspace{0.5em}
	\item \ldots extra noise may make filtering more conservative,
...
\begin{frame}{Solution IV: Sophisticated multiple imputation}
  ...
  \uncover<4->{%
  \begin{itemize}
  	\item<4-> Looks exactly like Gibbs sampling, but implied joint dist. may not exist
  	\vspace{0.5em}
  	\item<5-> Surprising lack of ML libraries (e.g., MICE in R)
  	\vspace{0.5em}
  	\item<6-> Slow!
  \end{itemize}
  }
...
\uncover<2->{%
\begin{align*}
  \ell(\*\theta) &= \sum_{i=1}^{N_1} \log p\left(\left(g_1^{(i)}, g_2^{(i)}, \*?\right); \*\theta\right) + \sum_{i=1}^{N_2} \log p\left(\left(g_1^{(i)}, \*?, g_3^{(i)}\right); \*\theta\right)\uncover<3->{\\
...
\begin{enumerate}[(I)]
  ...
  \vspace{0.7em}
  \item {\color{colneg}Trivial single imputation}
  \vspace{0.7em}
